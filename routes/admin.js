...
const ROLE_PERMISSIONS = {
  admin:           ['all'],
  vice_principal:  ['all'],
  teacher:         ['announcements','nav','campus','settings','leaves','inspections','points','salary','notifications'],
  limited_teacher: ['announcements','inspections','points'],
}
...
function requirePerm(perm) {
  return (req, res, next) => {
    ...
    return res.status(403).json({ success: false, message: '你的帳號沒有此功能的權限' })
  }
}
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (user && bcrypt.compareSync(password, user.password)) {
      ...
      return res.json({ success: true, role: 'admin', name: user.username })
    }
    // 再查教師
    const tResult = await pool.query('SELECT * FROM teachers WHERE username=$1', [username])
    const teacher = tResult.rows[0]
    if (teacher && await bcrypt.compare(password, teacher.password_hash)) {
      req.session.userId = 'teacher_' + teacher.id
      req.session.username = teacher.username
      req.session.userRole = teacher.role
      ...
      return res.json({ success: true, role: teacher.role, name: teacher.name })
    }
...
router.get('/check-auth', (req, res) => {
  if (req.session && req.session.userId)
    res.json({
      ...
      name: req.session.teacherName || req.session.username,
      teacherClass: req.session.teacherClass || null
    })
  else
...
// ── 後台管理：薪資管理 ────────────────────────────────────
router.get('/salary/all', requireAuth, async (req, res) => {
  const pool = req.app.locals.pool
...
router.put('/salary/base', requireAuth, async (req, res) => {
...
router.post('/salary/bonus', requireAuth, async (req, res) => {
...
router.delete('/salary/bonus/:id', requireAuth, async (req, res) => {
  const pool = req.app.locals.pool
  try {
...
// ── 榮譽榜 CRUD（後台）────────────────────────────────────
router.get('/honors', requirePerm('honors'), async (req, res) => {
  const pool = req.app.locals.pool
  try {
...
router.post('/honors', requirePerm('honors'), async (req, res) => {
...
router.put('/honors/:id', requirePerm('honors'), async (req, res) => {
  const { title, category, student_name, class_name, award, description, award_date } = req.body
  const pool = req.app.locals.pool
  try {
    await pool.query(
...
router.delete('/honors/:id', requirePerm('honors'), async (req, res) => {
  const pool = req.app.locals.pool
  try {
