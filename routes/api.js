...
// ── 學生課堂系統（公開 API）────────────────────────────────
// 學生登入（帳號密碼皆為學號）
router.post('/class/login', async (req, res) => {
  const { student_id, password } = req.body
  const pool = req.app.locals.pool
  try {
    const r = await pool.query('SELECT * FROM students WHERE student_id=$1', [student_id])
    if (r.rows.length === 0 || password !== student_id)
      return res.json({ success: false, message: '學號或密碼錯誤' })
    req.session.studentId = r.rows[0].student_id
    req.session.studentName = r.rows[0].name
    req.session.studentClass = r.rows[0].class_name
    res.json({ success: true, student: { student_id: r.rows[0].student_id, name: r.rows[0].name, class_name: r.rows[0].class_name } })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})

router.get('/class/me', (req, res) => {
  if (!req.session.studentId) return res.json({ success: false })
  res.json({ success: true, student: { student_id: req.session.studentId, name: req.session.studentName, class_name: req.session.studentClass } })
})
...
router.post('/class/join', async (req, res) => {
  ...
  const pool = req.app.locals.pool
  try {
    const r = await pool.query('SELECT * FROM classrooms WHERE code=$1', [code.toUpperCase()])
    if (r.rows.length === 0) return res.json({ success: false, message: '課堂代碼不存在' })
    await pool.query(
      'INSERT INTO classroom_members (classroom_id,student_id) VALUES ($1,$2) ON CONFLICT DO NOTHING',
      [r.rows[0].id, req.session.studentId]
    )
    res.json({ success: true, classroom: r.rows[0] })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})

// 學生的課堂列表
router.get('/class/classrooms', async (req, res) => {
  if (!req.session.studentId) return res.status(401).json({ success: false })
  const pool = req.app.locals.pool
  try {
    const r = await pool.query(`
      SELECT c.*, t.name AS teacher_name FROM classrooms c
      JOIN classroom_members cm ON cm.classroom_id = c.id
      JOIN teachers t ON t.id = c.teacher_id
      WHERE cm.student_id=$1 ORDER BY c.created_at DESC`, [req.session.studentId])
    res.json({ success: true, data: r.rows })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})

// 課堂作業（學生視角）
router.get('/class/classrooms/:id/assignments', async (req, res) => {
  if (!req.session.studentId) return res.status(401).json({ success: false })
  const pool = req.app.locals.pool
  try {
    const r = await pool.query(`
      SELECT a.*, sub.content, sub.score, sub.feedback, sub.submitted_at
      FROM assignments a
      LEFT JOIN assignment_submissions sub ON sub.assignment_id = a.id AND sub.student_id=$1
      WHERE a.classroom_id=$2 ORDER BY a.created_at DESC`, [req.session.studentId, req.params.id])
    res.json({ success: true, data: r.rows })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})

// 學生繳交作業
router.post('/class/assignments/:id/submit', async (req, res) => {
  if (!req.session.studentId) return res.status(401).json({ success: false })
  ...
  try {
    await pool.query(
      `INSERT INTO assignment_submissions (assignment_id,student_id,content)
       VALUES ($1,$2,$3) ON CONFLICT (assignment_id,student_id)
       DO UPDATE SET content=$3, submitted_at=NOW()`,
      [req.params.id, req.session.studentId, content]
    )
    res.json({ success: true })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})
...
// 課堂留言（學生）
router.get('/class/classrooms/:id/messages', async (req, res) => {
  if (!req.session.studentId) return res.status(401).json({ success: false })
  const pool = req.app.locals.pool
  try {
...
router.post('/class/classrooms/:id/messages', async (req, res) => {
  if (!req.session.studentId) return res.status(401).json({ success: false })
  const { content } = req.body
  ...
  try {
    const r = await pool.query(
      ...
      [req.params.id, req.session.studentId, req.session.studentName, 'student', content.trim()]
    )
...
// 學生登出
router.post('/class/logout', (req, res) => {
  req.session.studentId = null
  req.session.studentName = null
  req.session.studentClass = null
  res.json({ success: true })
})
